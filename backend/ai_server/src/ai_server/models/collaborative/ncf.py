...
class NCFRecommender(BaseRecommender):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not HAS_TENSORFLOW:
            raise ImportError(
                "TensorFlow is required for NCF model. Please install tensorflow>=2.0"
            )
    ...
    def fit(
        ...
        loguru.logger.info(
            f"Training NCF model with {len(interaction_data)} interactions..."
        )
    ...
    def _build_model(self) -> Any:  # keras.Model
        ...
        prediction = layers.Dense(1, activation="sigmoid", name="prediction")(
            concat_output
        )
        ...
        for user_idx in data["user_idx"].unique():
            ...
            for _ in range(n_negative):
                ...
                negative_samples.append(
                    {"user_idx": user_idx, "item_idx": neg_item, "rating": 0.0}
                )
    ...
    def predict_score(
        ...
        scores = self.model.predict(
            [np.array(user_indices), np.array(item_indices)], verbose=0
        )
