...
class TFIDFRecommender(BaseRecommender):
    ...
    def fit(self, interaction_data: pd.DataFrame,
        ...
        if self.tfidf_vectorizer is not None and self.tfidf_vectorizer.vocabulary_ is not None:
    ...
    def _build_user_profiles(self, interaction_data: pd.DataFrame,
                             item_content: pd.DataFrame) -> Dict[int, csr_matrix]:
        ...
        item_idx_to_row = {int(item_content.iloc[i]['item_idx']): i
    ...
    def predict(self, user_ids: Union[List, np.ndarray, str],
        ...
        """
        if not self.is_fitted or self.item_features_matrix is None or self.user_profiles is None:
        ...
        for user_id in user_ids:
            ...
            scores = cosine_similarity(user_profile_array, self.item_features_matrix).flatten()
